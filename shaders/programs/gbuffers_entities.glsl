...
void main() {
  ...
  tmpCd = color*texture2D(gcolor, midcoord);
    mixColor = tmpCd.rgb;
    avgDiv += tmpCd.a;
  tmpCd = color*textureOffset(gcolor, midcoord, ivec2(2, 2) );
...
// localShadowOffset is distance offset from surface to sample shadow
  ...
  // -- Consistent Night and Cave Darkness for Entities --
  float skyBrightness = clamp(luv.y * 1.1 - 0.1, 0.0, 1.0);
#ifdef OVERWORLD
  float nightPhaseMod = clamp((1.0 - sunPhaseMult) * 2.0, 0.0, 1.0);
#else
  float nightPhaseMod = 0.0;
#endif
  float darkFloorVal = mix(0.15, 0.08, nightPhaseMod); 
  outCd.rgb *= mix(vec3(darkFloorVal + LightBlackLevel * 0.1), vec3(1.0), skyBrightness * 0.9 + 0.1);
  ...
  // Mark entities in gaux1.b for post effects (white outline)
