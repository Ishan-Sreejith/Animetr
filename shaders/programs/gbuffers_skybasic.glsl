...
const vec3 fogColorNight = vec3(0.001, 0.0015, 0.004); // Deep Indigo/Black
...
if( BaseQuality > 0 ){
  ...
  #if CandyMode == 1
    // Smooth Candy Sky Gradient
    vec3 skyTop = vec3(0.75, 0.6, 1.0); // Lavender-Indigo Zenith (Purple)
    vec3 skyBottom = vec3(1.0, 0.85, 0.9); // Peach-Pink Horizon
    
    // Create a smooth transition based on elevation (upDot)
    vec3 skyCandy = mix(skyBottom, skyTop, smoothstep(0.0, 0.85, upDot));
    
    // Add atmospheric glow near horizon
    float horizonGlow = 1.0 - smoothstep(0.0, 0.45, upDot);
    skyCandy = mix(skyCandy, skyBottom * 1.05, horizonGlow * 0.4);
    
    outCd.rgb = mix(outCd.rgb, skyCandy * (skyColor*0.5+0.7), (1.0 - rainStrength) * 0.6);
  #endif
