...
void main() {
  ...
  // Leaf sway (trees)
  if (mc_Entity.x == 810 || mc_Entity.x == 8101 || mc_Entity.x == 801) {
    float t = frameTimeCounter * LeafSwaySpeed;
    float phase = (localVertex.x + localVertex.z) * 0.5;
    float sway = sin(t + phase) * LeafSwayStrength;
    localVertex.x += sway;
    localVertex.z += sway * 0.7;
    localVertex.y += sin(t + phase * 1.3) * (LeafSwayStrength * 0.5);
  }
  //vec3 position = mat3(gbufferModelView) * basePos + gbufferModelView[3].xyz;
  //vec3 position = (gbufferModelView * vec4(basePos,1.0)).xyz;
  vec3 position = (gbufferModelView * localVertex).xyz;
	
  vPos = vec4(position,1.0);
  vLocalPos = localVertex.xyz;//basePos;
  ...
  tmpCd = vColor * texture(gcolor, midcoord);
    mixColor = tmpCd.rgb;
  #if (BaseQuality > 1)
  tmpCd = vColor * textureOffset(gcolor, midcoord, ivec2(-2, 2) );
...
// Flowers; For more distinct colors
  ...
  if( mc_Entity.x == 811 ){ // Dirt Grass
    ...
    vDeltaMult=50.5;
		vDeltaPow=4.53;
    // Reduce orange/warm cast in grass
    vColor.rgb = mix(vColor.rgb, vColor.rgb * vec3(0.85, 1.0, 0.95), 0.7);
    vAvgColor.rgb = mix(vAvgColor.rgb, vAvgColor.rgb * vec3(0.85, 1.0, 0.95), 0.7);
    ...
    vDeltaPow=.50;
		...
		vDeltaMult=15.0; // High detail for standard blocks
    vDeltaPow=1.20;
	}else if( mc_Entity.x == 104 || mc_Entity.x == 1041 || mc_Entity.x == 303 ){ // Ores (Stone, Deepslate, Nether)
    // Make Ores POP: Extremely high detail and subtle cave glow
		vDeltaMult=12.0; 
    vDeltaPow=2.5; 
		vAvgColor=vec4(0.4,0.4,0.4,1.0);
    vDepthAvgColorInf = 0.2; // Keep original texture details mostly
    vCdGlow = EnableOreGlow ? 0.15 : 0.0; // Add a subtle shimmer glow
...
void main() {
  ...
  // Use sky-light component (luv.y) to detect if we are outdoors or in a cave
	float skyBrightness = clamp(luv.y * 1.1 - 0.1, 0.0, 1.0);
...
// -- -- -- -- -- -- -- -- -- -- -- -- --
	...
	// Ultra-Dark Caves & Night: Near-zero ambient light in deep spelunking or midnight
  float nightPhaseMod = clamp((1.0 - sunPhaseMult) * 2.0, 0.0, 1.0);
	float skyBrightMultFit = mix(1.0, 1.0 - skyBrightness * 0.15, (1.0 - frozenSnowGlow));
	outCd.rgb *= skyBrightMultFit;
		
  float darkFloorVal = mix(0.08, 0.03, nightPhaseMod); // Brighter in caves
	outCd.rgb *= mix(vec3(darkFloorVal + LightBlackLevel * 0.15), lightCd.rgb, skyBrightness * 0.85 + 0.15);
  
  // Extra night-time pull (Outdoors only) - reduced for better cave visibility
  outCd.rgb *= mix(1.0, 1.0 - NightDarkness * 0.5, nightPhaseMod * skyBrightness);
