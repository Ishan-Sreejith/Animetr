...
// -- -- -- -- -- -- -- --
  ...
  if (depthBase < 0.9999) {
    for (int x = -1; x <= 1; x++) {
      for (int y = -1; y <= 1; y++) {
        if (x == 0 && y == 0) continue;
        vec2 o = vec2(x, y) * texelSize * BorderThickness;
        float nd = texture2D(colortex1, uv + o).r;
        vec3 nn = texture2D(colortex2, uv + o).rgb * 2.0 - 1.0;
        float depthDiff = nd - depthBase;
        float isSkyNeighbor = step(0.95, nd) + step(nd, AirDepthCutoff);
        float isSkyCurrent = step(0.95, depthBase) + step(depthBase, AirDepthCutoff);
        float edgeAir = step(DepthThreshold, abs(depthDiff)) * step(0.5, isSkyNeighbor) * (1.0 - step(0.5, isSkyCurrent));
        edgeAirMax = max(edgeAirMax, edgeAir);
      }
    }
  }
  // Only outline where geometry meets air/background
...
// -- -- -- -- -- -- -- -- -- --
  ...
  outCd.rgb = mix(outCd.rgb, waterTarget, clamp(waterMask * 0.7, 0.0, 1.0));
...
// Optional extra detail (restore texture detail)
  ...
  outCd.rgb = mix(outCd.rgb, baseCd.rgb, ExtraDetailStrength);
#endif
...
outCd.rgb = mix(outCd.rgb, outlineCol, clamp(softEdge, 0.0, 1.0));
